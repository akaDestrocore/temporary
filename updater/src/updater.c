...
void nextion_sendProgress(uint8_t percent);
void nextion_sendStatus(const char *pMsg);
...
int main(void) {
    ...
    // -- Nextion --------------------------------------------------
    cfg.nextion.pUsart = USART3;
    cfg.nextion.pGpio_port = GPIOB;
    cfg.nextion.rx_pin_num = 11U;
    cfg.nextion.tx_pin_num = 10U;
    cfg.nextion.gpio_af = 7U;
    cfg.nextion.gpio_ahb1_bits = RCC_AHB1ENR_GPIOBEN;
    cfg.nextion.apbx_bit = RCC_APB1ENR_USART3EN;
    cfg.nextion.is_apb2 = 0U;
    cfg.nextion.apb_freq_hz = 48000000U; // 96 MHz HCLK / PPRE1_DIV2
    cfg.nextion.baud_rate = 9600U;
    cfg.nextion.irqn = USART3_IRQn;
    cfg.nextion.nvic_priority = 8U;
    cfg.nextion.ack_timeout_ms = 200U;
...
Updater_Err_e updater_init(const Updater_Config_t *pCfg) {
    ...
    (void)nextion_init(&gCfg.nextion);
...
/**
  * @brief Nextion ekranında bir ilerleme çubuğu günceller ve gönderim
  *        tamamlanana kadar bloklar.
  * @param percent 0-100 arası ilerleme değeri.
  * @retval None
  */
void nextion_sendProgress(uint8_t percent) {

    char buff[NEXTION_CMD_MAX_LEN];
    uint32_t i = 0U;

    const char *pPrefix = "j0.val=";
    while ('\0' != pPrefix[i]) {
        buff[i] = pPrefix[i];
        i++;
    }
    ...
    if (percent >= 100U) {
        ...
        buff[i++] = (char)('0' + (percent / 10U));
        buff[i++] = (char)('0' + (percent % 10U));
    } else {
        buff[i++] = (char)('0' + percent);
    }

    buff[i] = '\0';

    (void)nextion_sendCmd(buff);

    uint32_t deadline = HAL_GetTick() + gCfg.nextion.ack_timeout_ms + 50U;
    while (nextion_isBusy()) {
        if (HAL_GetTick() > deadline) {
            break; // ACK gelmedi - ilerlemeye devam
        }
    }
}

/**
  * @brief Nextion ekranında bir status yazısını günceller.
  * @param pMsg NULL karakterle biten mesaj işaretçisi.
  * @retval None
  */
void nextion_sendStatus(const char *pMsg) {
    ...
    char *pPrefix = "t0.txt=\"";
    while (('\0' != pPrefix[i]) && (i < (NEXTION_CMD_MAX_LEN - 2U))) {
        buff[i] = pPrefix[i];
        i++;
    }

    uint32_t j = 0U;
    while (('\0' != pMsg[j]) && (i < (NEXTION_CMD_MAX_LEN - 2U))) {
        buff[i] = pMsg[j];
        i++;
        j++;
    }

    if (i < (NEXTION_CMD_MAX_LEN - 1U)) {
        buff[i++] = '"';
    }
    buff[i] = '\0';
    (void)nextion_sendCmd(buff);
